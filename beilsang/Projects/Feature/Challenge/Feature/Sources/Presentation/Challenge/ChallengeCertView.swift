...
public struct ChallengeCertView: View {
    private enum PendingVerificationAction {
        case submit
        case retry
    }
    ...
    @State private var pendingVerificationAction: PendingVerificationAction?
    ...
    public var body: some View {
        ...
        .sheet(isPresented: $viewModel.showVerificationSheet, onDismiss: handleVerificationSheetDismiss) {
            PhotoVerificationSheet(
                result: viewModel.verificationResult,
                failed: viewModel.verificationFailed,
                onConfirm: {
                    pendingVerificationAction = .submit
                    viewModel.dismissVerificationSheet()
                },
                onRetry: {
                    pendingVerificationAction = .retry
                    viewModel.dismissVerificationSheet()
                }
            )
        ...
        VStack(alignment: .leading, spacing: 12) {
            ...
            VStack(alignment: .leading, spacing: 6) {
                CustomPhotoPickerView(
                    photos: $viewModel.selectedPhotoStates,
                    maxPhotoCount: 5,
                    onAddTapped: { viewModel.showImageSelector() },
                    onImageTapped: { id in viewModel.showImageModal(id: id) },
                    onRemoveTapped: { id in viewModel.removeImage(id: id) }
                )
    ...
    private var submitButton: some View {
        Button {
            ...
            .frame(height: 60)
            .background(
                viewModel.canSubmit || viewModel.isVerifying
    ...
    private var gradientBackground: some View {
        LinearGradient(
            colors: [ColorSystem.labelWhite.opacity(0), ColorSystem.labelWhite],
            startPoint: .top,
            endPoint: .bottom
        )
        .frame(height: 120)
    ...
    private func handleSubmit() {
        ...
        Task { await viewModel.verifyBeforeSubmit() }
    }

    /// 검증 시트가 완전히 닫힌 뒤 실행: 시트 dismiss와 화면 dismiss가 충돌하지 않도록 보장
    private func handleVerificationSheetDismiss() {
        guard let action = pendingVerificationAction else { return }
        pendingVerificationAction = nil

        switch action {
        case .submit:
            handleConfirmedSubmit()
        case .retry:
            viewModel.showImageSelectorForRetry()
        }
    }

    private func handleConfirmedSubmit() {
        Task { @MainActor in
            let success = await viewModel.submitCertification()
            if success {
                toastManager.show(iconName: "toastCheckIcon", message: "챌린지 인증이 완료되었습니다")
                dismiss()
            } else {
                toastManager.show(iconName: "toastWarningIcon", message: "인증 처리 중 오류가 발생했습니다")
            }
        }
    }
}
